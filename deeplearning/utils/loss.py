...
class SISDRLoss(torch.nn.Module):
    ...
    Shape:
        - input : :math:`(batch, time, nchs)`.
        - target: :math:`(batch, time, nchs)`.
    """
...
class CombinedLoss(torch.nn.Module):
    """结合SI-SDR和多分辨率STFT的损失函数
    
    Args:
        sisdr_weight (float): SI-SDR损失的权重
        stft_weight (float): 多分辨率STFT损失的权重
        fft_sizes (list): FFT大小列表
        hop_sizes (list): 跳跃大小列表
        win_lengths (list): 窗口长度列表
        sisdr_zero_mean (bool): 是否在SI-SDR中移除DC偏移
        eps (float): 数值稳定性常数
    """
    def __init__(
        self,
    ...
    def forward(self, input, target):
        """
        Args:
            input: 预测信号 [batch, time, channels]
            target: 目标信号 [batch, time, channels]
        ...
        # 对于STFT损失，需要调整为[batch*channels, time]
        input = input.transpose(1, 2)
        target = target.transpose(1, 2)
        batch_size, n_channels, time_steps = input.shape
        stft_input = input.reshape(-1, time_steps)
        stft_target = target.reshape(-1, time_steps)
            
        si_sdr_loss = self.sisdr_loss(sisdr_input, sisdr_target)
        stft_loss = self.stft_loss(stft_input, stft_target)
        
        # 根据权重组合损失
        combined_loss = self.sisdr_weight * si_sdr_loss + self.stft_weight * stft_loss
